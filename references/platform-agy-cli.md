...
| Required handshake | When the shared protocol requires a handshake, require a parent-visible `ACCEPTED` or blocked return and inspect it before execution. |
| Continue original executor | Continue the same conversation through the active continuation interface. If its identity cannot be preserved, return `MISSING_CAPABILITY`; a terminated target requires a new packet. |
