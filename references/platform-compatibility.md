...
The root `SKILL.md` is the semantic source. Platform maps translate only native creation, handshake, continuation, reception, stopping, and runtime permission outcomes. Shared dependency handoff, failure labels, authorization, ownership, and verification remain in [protocol.md](protocol.md). Use the active platform map at dispatch time; do not copy platform schemas or shared protocol text into compatibility rules.
