...
Delivering something other than the requested form, because producing the requested form would have been false, is also `PARTIAL` only when execution has begun and the substitute remains inside the packet's existing scope, permissions, and return contract. `Requested-form substitution` names the form not produced, why producing it would have been false, and the in-scope substitute actually delivered. Matching the requested form is part of the agreed minimum deliverable, and an unrequested substitute never upgrades the status to `DONE`. `PARTIAL` classifies the result; it does not authorize a new artifact shape or other boundary change. If the need for a different form is known before execution, return `BLOCKED` and require a new packet. If it emerges after execution starts but producing the substitute would change scope, permissions, ownership, or the return contract, do not produce it; return `PARTIAL` for the safe completed scope and make a new packet the safest next action.
